...
Так же можно сравнить строки, списки и т.д. К примеру:
```
print('abc' != 'aaa')  # True
print([1, 2] == [1, 2])  # True
print((0, 0) > (1, 2))  # False
...
Это не все операторы Python, но самые основные. Есть ещё битовые операторы в Python, которые работают с отдельными битами чисел и выполняют побитовые операции. Но, в данном предмете они рассмотрены не будут. Перед тем как перейти к заданиям, обязательно ознакомьтесь с кодом в файлах.
